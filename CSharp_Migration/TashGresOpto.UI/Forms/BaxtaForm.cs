...
public partial class BaxtaForm : Form
{
    private readonly OptoDbContext _context;
    private readonly FuelCalculator _fuelCalculator;
    private DateTime _selectedDate;
    private int _calculationType; // 1 = Первый, 2 = Повторный

    public BaxtaForm(OptoDbContext context, FuelCalculator fuelCalculator)
    {
        _context = context;
    ...
    {
        this.Text = "ОПТО ТашТЭС - Расчет по вахтам";
        ...
        lblBlockHeader.Text = "N БЛОКА";
        lblShiftHeader.Text = "СМЕНА";
        lblCountersHeader.Text = "ПОКАЗАНИЯ СЧЕТЧИКОВ";
        lblOutputHeader.Text = "ВЫРАБ.";
        lblSNHeader.Text = "С.Н.";
        
        // Заголовки технологических параметров
        lblParamsTitle.Text = "ТЕХНОЛОГИЧЕСКИЕ ПАРАМЕТРЫ";
        ...
        lblTn.Text = "Темп. нар. воздуха:";
        
        // Результаты
        lblResultsTitle.Text = "РЕЗУЛЬТАТЫ РАСЧЕТА";
    ...
    private void SetupCountersGrid()
    {
        ...
        dgvCounters.Columns[0].Width = 100;
        dgvCounters.Columns[1].Width = 100;
    ...
    {
        // Открыть форму просмотра графика вахт (процедура SEEBAX, строка 2055)
        using (var scheduleForm = new BaxtaScheduleForm(_context))
        {
    ...
    {
        var result = MessageBox.Show("Количество экземпляров для печати?", 
            "Печать", MessageBoxButtons.OK, MessageBoxIcon.Question);
    ...
    private void btnFirstCalc_Click(object sender, EventArgs e)
    {
        ...
        // Проверка: был ли уже первый расчет за эту дату
        int mmdd = int.Parse(_selectedDate.ToString("MMdd"));
        ...
        if (existing)
        {
            ...
            {
                // Ошибка - позволить исправить дату
                return;
            }
            else
            {
                // Переключить на повторный расчет
                _calculationType = 2;
            }
        }
    ...
    // === ОБРАБОТЧИКИ ФОРМЫ ВВОДА ДАННЫХ ===
    ...
    private void btnCalculate_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            foreach (DataGridViewRow row in dgvCounters.Rows)
            {
                ...
                {
                    Nagruzka = output / 8.0, // примерная нагрузка
                    Wyrabotka = output,
            ...
            if (totalOutput == 0)
            {
                MessageBox.Show("Введите данные в таблицу счетчиков!", "Ошибка", 
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            
            // Повторный расчет для отображения результатов
            var firstRow = dgvCounters.Rows[0];
    ...
    private void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                BlockNumber = 1, // или из первой строки
                Date = _selectedDate,
                Smena = 1,
                TIM = (double)numTim.Value,
                DRO = (double)numDro.Value,
